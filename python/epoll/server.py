...
# more than one connection at a time by using select(). The new version starts
# out by creating a non-blocking TCP/IP socket and configuring it to listen on
# an address.
...
while inputs:
    ...
    # the lists passed in. All of the sockets in the readable list have
    ...
    for s in readable:
        ...
        # The next case is an established connection with a client that has
        # sent data. The data is read with recv(), then placed on the queue so
    ...
    # next time through the loop select() does not indicate that the socket is
    # ready to send data.
